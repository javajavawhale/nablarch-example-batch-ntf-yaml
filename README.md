...
> **このリポジトリは [nablarch/nablarch-example-batch](https://github.com/nablarch/nablarch-example-batch) のフォークです。**
>
...
* 都度起動バッチ
    ...
    * データバインドを使用した住所登録バッチ
        * 指定されたフォルダ内の住所情報CSVファイルを、データバインドを使用して読み込み、`ZIP_CODE_DATA`テーブルに登録します。
    * 汎用データフォーマットを使用した住所登録バッチ
        * 指定されたフォルダ内の住所情報CSVファイルを、汎用データフォーマットを使用して読み込み、`ZIP_CODE_DATA`テーブルに登録します。
* 常駐バッチ
    * PDF読み込みバッチ
        * 指定されたフォルダ内のpdfファイルを読み込み、`FILE_DATA`テーブルに登録した後、ファイルを削除します。
